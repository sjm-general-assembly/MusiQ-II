...
musiqApp_getYoutubeSearchResults = function(title, callback) {
...
musiqApp_formatSearchResults = function(responseJSON) {
  ...
  // signal search list has changed, (triggers ui update)
...
musiqApp_setCurrentSongToPlay = function() {
  ...
  var foundSong = Requests.findOne({status: musiqApp_STATUS_NOW_PLAYING});
  if (foundSong !== undefined) {
    ...
    foundSong = Requests.findOne({status: musiqApp_STATUS_WAITING}, {sort: {created: 1}});
    if (foundSong !== undefined) {
      currentSongId = foundSong.videoId;
...
musiqApp_cueNextSong = function() {
  ...
  var playedSong = Requests.findOne({status: musiqApp_STATUS_NOW_PLAYING});
